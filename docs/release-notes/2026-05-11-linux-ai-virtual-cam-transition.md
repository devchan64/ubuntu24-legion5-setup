...
`media` 도메인에서 기존 OBS 가상 오디오 유닛 생성 기능을 제거하고 `ai-virtual-cam`(Linux `v4l2loopback` 경로) 설치 단계로 전환했습니다.

## 주요 변경사항

- `scripts/cmd/media.sh`
  - 오디오 설치 스텝을 `media:audio:virtual:install`에서 `media:audio:ai-virtual-cam:install`으로 변경
  - 실행 스크립트를 `scripts/media/audio/install-ai-virtual-cam.sh`로 교체
- `scripts/media/audio/install-virtual-audio.sh` 삭제
- `scripts/media/audio/install-ai-virtual-cam.sh` 추가
...
- Linux 카메라 처리 경로를 단일 도구(`ai-virtual-cam`)로 표준화합니다.
- 기존 사용자 systemd 유닛 및 pactl 모듈 의존도를 제거해 관리 복잡도를 낮춥니다.

## 검증

- `bash -n scripts/cmd/media.sh scripts/media/audio/install-ai-virtual-cam.sh`
